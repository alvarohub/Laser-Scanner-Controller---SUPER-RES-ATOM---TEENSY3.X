...
uint8_t Laser::myID = 1; // we should start by 1 (I will use this for trigger selection, and reserve 0 for the external trigger)
...
void Laser::setCarrierMode( bool _carrierMode) { // note: the carrier is a square wave (pwm, 50% or adjusted at 58% or so
...
bool Laser::setToCurrentState() {
		...
		return(myState.state); // return the switch state (ON/OFF)
}
