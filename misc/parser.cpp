...
bool CmdDictionnary::interpretCommand(const String _cmd, uint8_t _numArgs, StackArgs _cmdArgs)
{
	...
	{
		if (cmdArray[indexCmd].tryCmd(_cmd, _numArgs, _cmdArgs))
			return true;
	}
	return false;
}
...
bool CmdDictionnary::isInDictionnary(const char *_cmd, uint8_t _numArgs)
{
	for (uint8_t indexCmd = 0; indexCmd < numCmds; indexCmd++)
	{
		if (cmdArray[indexCmd].checkCmd(_cmd, _numArgs))
			return true;
	}
	return false;
}
...
{
	// NOTE: _rawString is supposed to be a well fomed C string
	...
	enum stateParser
	{
		...
		NUMBER,
		CMD
	} myState; //Note: the name of an unscoped enumeration may be omitted: such declaration only introduces the enumerators into the enclosing scope.
	myState = START;

	for (readingHead = 0; readingHead <= strlen(copyRawString); readingHead++)
	{ // note: we include the c_string terminator!
		...
		if ((val >= '-') && (val <= '9') && (val != '/'))
		{ // this is 45 to 57 (including '-', '/' and '.')
		...
		{ // this means that we received '/' or 0, which can terminate either a number ('/') or a command ('/' or 0)
			if (myState == NUMBER)
			{
				...
				PRINT(F(" END ARG: "));
				PRINT_CSTRING(argStack[numArgs]);
				numArgs++;
			}
			else if (myState == CMD)
			{ // this means we completed the reception of a command: process right away
				cmdString[indexCmdString] = '\0';
				indexCmdString = 0;

				PRINTLN();
				PRINT(F("  END COMMAND: "));
				PRINT_CSTRING(cmdString);
				PRINT(F("  NUM ARGS: "));
				PRINTLN(numArgs);

				if (!myCmdDic->interpretCommand(cmdString, numArgs, argStack))
					result = false;
